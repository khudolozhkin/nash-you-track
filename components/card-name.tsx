...
export default function CardName({card, cardId}: {card: Card, cardId: string}) {
  ...
  return (
    <>
      <input placeholder="Название карточки..." ref={input} onBlur={onBlur} onKeyDown={(e) => {handleKeyDown(e)}} onChange={onChange} maxLength={32} className="mb-[2px] font-bold text-3xl h-[42px] outline-none bg-general-background dark:bg-brand-background-dark" size={value.length}/>
    </>
  )
}